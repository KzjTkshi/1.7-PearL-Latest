        ...
        if(checkpointid == CP[x])
        {
            if(UnderAttack[x] == 1)
         	...
         	else
         	{
            	...
            	SendClientMessage(playerid, COLOR_YELLOW,"Tunggu 60 detik untuk menangkap zona ini");
         	}
		}
	}
 	return 1;
			...
			if(gTeam[playerid] == TEAM_USA)
			{
				if(IsPlayerInAnyVehicle(killerid))
				{
					if(GetVehicleModel(GetPlayerVehicleID(killerid)) == 432 || GetVehicleModel(GetPlayerVehicleID(killerid)) == 520 || GetVehicleModel(GetPlayerVehicleID(killerid)) == 425 || GetVehicleModel(GetPlayerVehicleID(killerid)) == 447)
					{
						...
						GameTextForPlayer(killerid,"~y~Tidak Diizinkan", 3000, 3);
...
CMD:cmds(playerid, params[])
{
	...
	format(msg, 400, "{FF0000}%s Untuk informasi lebih lanjut silahkan baca /rules dan /help. Terima kasih.", msg);
...
CMD:help(playerid, params[])
{
	...
	format(msg, 400, "{09FF00}%s• /pm -> {FFFFFF}Jika Anda ingin berbicara dengan seseorang secara pribadi, Anda dapat menggunakan ini.\n", msg);
	format(msg, 400, "{09FF00}%s• /admins -> {FFFFFF}Menunjukkan admin online dan On-Duty saat ini.\n", msg);
	format(msg, 400, "{FF0000}%s Jika Anda tidak yakin tentang sesuatu, jangan ragu untuk bertanya secara online /admins.\n\n", msg);
...
CMD:rules(playerid, params[])
{
	...
	format(msg, 400, "{FF0000}%s Pelanggaran aturan yang terus-menerus di bawah ini akan menyebabkan tindakan yang relevan segera diambil.", msg);
...
CMD:changelogs(playerid, params[])
{
	...
	format(msg, 400, "{09FF00}%s • Penambahan TEAM BRITAN\n", msg);
	format(msg, 400, "{09FF00}%s • Perubahan Versi Server Dari w1.2 --> 1.7 PearL Latest\n", msg);
	format(msg, 400, "{09FF00}%s • Penambahan Base Britan Dan Kendaraan\n\n", msg);
	format(msg, 400, "{FF0000}%s • Penambahan Perintah Baru Ke Admin", msg);
...
CMD:givecash(playerid, params[])
{
	...
	if(targetid == INVALID_PLAYER_ID)
		return SendErrorMessage(playerid, "ID PEMAIN TIDAK BENAR!");
...
CMD:setcash(playerid, params[])
{
	...
	AdminSendMessage(COLOR_RED, "AdmCmd: %s Telah Tetapkan %s Uang Ke $%s", WorldWar5CountryData[playerid][pUCP], GetName(userid), FormatNumber(strcash(amount)));
	return 1;
}
...
CMD:setskin(playerid, params[])
{
	...
	if (skinid < 0 || skinid > 311)
	    return SendErrorMessage(playerid, "ID KULIT HANYA ADA DARI 0 SAMPAI 311.");
...
CMD:unjail(playerid, params[])
{
	...
	if(WorldWar5CountryData[targetid][pJailTime] < 1)
	    return SendErrorMessage(playerid, "Pemain itu tidak dipenjara!");
...
CMD:freeze(playerid, params[])
{
	...
	SendServerMessage(playerid, "Kamu telah membekukan gerakan %s.", GetName(userid));
	return 1;
}
...
CMD:unfreeze(playerid, params[])
{
	...
	SendServerMessage(playerid, "Anda telah mencairkan gerakan %s.", GetName(userid));
	return 1;
}
...
CMD:unban(playerid, params[])
{
	...
	new kontol[178];
	mysql_format(sqlww5c, kontol, sizeof(kontol), "SELECT * FROM `banned` WHERE `UCP` = '%s' LIMIT 1", name);
	mysql_tquery(sqlww5c, kontol, "OnUCPUnban", "ds", playerid, name);
	return 1;
}
...
CMD:ban(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_WHITE, "AdmCmd: Akun %s Telah Di Banned Oleh %s", WorldWar5CountryData[targetid][pUCP], WorldWar5CountryData[playerid][pUCP]);
	SendClientMessageToAllEx(COLOR_WHITE, "ALASAN: %s", reason);
	...
	new zstr[325];
	format(zstr, sizeof(zstr),"{FFFFFF}UCP Anda telah diblokir dari server ini\n{FF0000}Alasan: {FFFFFF}%s\n{FF0000}Di Blokir Oleh: {FFFFFF}%s\n{FF0000}Hari Blokir: {FFFFFF}%i/%02d/%02d %02d:%02d\n{FFFFFF}Jika Ingin UNBANNED AKUN Bisa Join Ke {FF0000}https://discord.gg/BfSBKmuxVF", reason, WorldWar5CountryData[playerid][pUCP]);
